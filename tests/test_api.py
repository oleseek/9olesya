...
# Проверка, что ответ можно распарсить как JSON
...
assert response_time < 1000, f"Response time {response_time:.2f}ms exceeds
1000ms"
...
assert re.match(r'^[^@\s]+@[^@\s]+\.[^@\s]+$', email), f"Invalid email format:
{email}"
...
"""Проверка наличия обязательных полей в созданном пользователе"""
post_data = {
"name": "Test User",
"username": "testuser",
"email": "testuser@example.com"
...
for field in required_fields:
assert field in json_data, f"Missing required field: {field}"
def test_post_user_id_is_assigned(self):
...
"""Проверка, что отправленные данные сохранились в ответе"""
post_data = {
"name": "Test User",
"username": "testuser",
"email": "testuser@example.com"
...
def test_post_response_has_correct_headers(self):
"""Проверка правильности заголовков ответа"""
post_data = {
"name": "Test User",
"username": "testuser",
"email": "testuser@example.com"
...
"""Проверка обновления пользователя (статус 200)"""
put_data = {
"name": "Updated User",
...
assert response.status_code == 200, f"Expected 200, got {response.status_code}"
def test_put_response_is_valid_json(self):
"""Проверка валидности JSON ответа при обновлении"""
put_data = {
"name": "Updated User",
"username": "updateduser",
"email": "updated@example.com"
...
"""Проверка, что ID остался прежним после обновления"""
put_data = {
"name": "Updated User",
"username": "updateduser",
"email": "updated@example.com"
...
"""Проверка, что отправленные данные сохранились при обновлении"""
put_data = {
"name": "Updated User",
"username": "updateduser",
"email": "updated@example.com"
...
def test_put_updated_user_has_required_fields(self):
"""Проверка наличия обязательных полей у обновленного пользователя"""
put_data = {
"name": "Updated User",
"username": "updateduser",
"email": "updated@example.com"
...
"""Проверка, что несуществующий пользователь возвращает 404"""
...
assert all(status == 200 for status in results), f"Not all requests succeeded:
{results}"
